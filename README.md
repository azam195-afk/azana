...
## Membuat ZIP refactor
Jalankan perintah berikut untuk membuat satu file ZIP berisi hasil refactor production-ready:

```bash
./scripts/create-refactor-zip.sh
```

Output akan dibuat di `dist/azana-refactor-production.zip` beserta checksum `dist/azana-refactor-manifest.txt`.

## Deploy ke Vercel
Repository ini sekarang menyertakan `vercel.json` dan script build statis supaya Vercel tidak salah membaca folder `public/` sebagai output utama. Saat deploy, Vercel akan menjalankan:

```bash
npm run build
```

Hasil website siap publish akan berada di `dist/site`, lalu Vercel menyajikan folder tersebut sebagai output production.
